...
É nesse contexto que se insere o ecossistema digital proposto neste trabalho. As funcionalidades previstas (cadastro e gestão de associados, controle de mensalidades e organização de eventos) correspondem diretamente às lacunas identificadas na literatura quanto à sistematização de processos em entidades tradicionalistas. O sistema busca, portanto, operacionalizar os benefícios descritos pela teoria dos sistemas de informação em um contexto organizacional específico, contribuindo para a redução do retrabalho, a centralização das informações e a melhoria do acesso dos associados às informações da entidade \citep{laudon2014,delone2003}.
...
Assim, ao propor um ecossistema digital entidade--associado, esta pesquisa insere-se na perspectiva de transformação digital aplicada a contextos culturais tradicionais, buscando demonstrar que inovação tecnológica e preservação identitária não são elementos excludentes, mas podem coexistir de forma complementar \citep{hinings2018,warnerwager2019}. Nesse sentido, a atenção ao grau de maturidade digital dos usuários torna-se um requisito de projeto, e não apenas uma consideração secundária: o ecossistema proposto prevê fluxos mediados pela entidade precisamente para assegurar que associados com menor familiaridade tecnológica não sejam excluídos do processo de digitalização, preservando a dimensão comunitária e inclusiva que caracteriza o movimento tradicionalista \citep{vial2019,hinings2018}.
...
Para orientar a análise dos resultados, serão adotados critérios objetivos que indicarão o grau de sucesso do ecossistema digital. Entre os principais critérios previstos estão: redução do retrabalho administrativo em comparação ao fluxo atual (planilhas e registros físicos); facilidade de uso percebida pelos participantes nos cenários de teste; clareza das informações apresentadas nas interfaces web e mobile; rapidez para consultar situação de mensalidades e realizar reservas; e aceitação geral do sistema pelos usuários da entidade. Esses critérios serão avaliados de forma combinada, cruzando os resultados do SUS com os dados coletados nas entrevistas e na observação, permitindo uma análise mais abrangente da viabilidade prática da solução \citep{brooke1996,creswell2014}.
